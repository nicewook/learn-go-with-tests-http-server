...
**[구현한 코드 보기(클릭)](https://github.com/quii/learn-go-with-tests/tree/main/http-server)**

플레이어가 얼마나 많은 게임을 이겼는지 확인할 수 있는 웹서버를 만들어야 한다. 

-   `GET /players/{name}` 는 전체 승점을 리턴해야 한다. 
-   `POST /players/{name}` 를 하나 보낼 때 마다 승점이 1씩 증가해야 한다. 

TDD로 가능한 빨리 동작하는 소프트웨어를 만든 다음, 목표로 하는 구현을 완료할 때까지 반복적으로 작은 개선을 해나갈 것이다. 이렇게 구현 하면

- 어떤 순간에도, 문제 발생 범위가 작게 유지된다
- rabbit holes(*실제 구현 이외의 사항에 너무 많은 시간을 빼앗기는 것)에 빠지지 않는다
- 문제가 발생하여 이전으로 되돌아가도 낭비한 작업이 적다

## 적색, 녹색, 리팩터 

이 책 내내, 테스트를 먼저 작성하고 실패하게 하고 (적색), 동작하는 _최소한_ 의 코드를 짠 다음 (녹색), 리팩터링하는 TDD 프로세스를 강조해왔다. 

최소한의 코드만을 짠다는 원칙은 TDD가 가져다주는 안전성의 측면에서 중요하다. "적색"에서 가능한 빨리 벗어나야 한다. 

켄트 벡은 이렇게 말했다.

> 무슨 수를 써서라도 테스트를 빨리 통과하라

테스트 통과를 위해 저지른 잘못들은 리팩터링으로 고쳐나가면 되며, 리팩터링은 테스트 덕분에 안심하고 진행할 수 있다. 

### 이렇게 하지 않는다면?

적색 상태에서 수정을 많이 하면 할 수록 테스트로 커버되지 않는 더 많은 문제가 추가되기 쉽다. 

그러니, 테스트를 통과하는 의미있는 코드를 조금씩, 반복적으로 짜라는 것이다. 이렇게 하면 몇 시간씩 래빗홀에 빠지지 않게 된다. 
